// GuiUtils
// $Id: GuiUtils.java,v 1.5 2005/01/29 05:49:39 gjuggler Exp $
//
...
public class GuiUtils
{
	...
	public static boolean checkVMVersion( java.awt.Component component )
	{
		...
		if( majorVersion < 2 && minorVersion < 4 )
		{
			...
			javax.swing.JOptionPane.showMessageDialog( component, GuiUtils.getStringLocalized("wrongJavaVersionMessage") + jvmVersion, 
																									GuiUtils.getStringLocalized("wrongJavaVersionTitle"), 
	 ...
	 * 	<BR>- get current clips table view
	 * 	<BR>- get oids for selected clips from the view
	...
	public static boolean showPlaylistPropertyDialog( Playlist playlist, Frontend frontend, Backend backend )
	{
		...
		// set tree
		DefaultTreeModel tree =		playlist.isAutoplaylist() 
															? new DefaultTreeModel(GuiUtils.getAutoPlaylistPropertyTreeModel()) 
															: new DefaultTreeModel(GuiUtils.getPlaylistPropertyTreeModel());
		...
		if( playlist != null )
		{
			textToSet = ( oids != null && !oids.isEmpty() )
									...
									? oids.size() + " of " + playlist.getTotalClips() + " " + GuiUtils.getStringLocalized("clips") 
										+ " | " + formatTime( myPod.getBackend().getTotalTime(oids) ) 
										+ " | " + formatFilesizeMB( myPod.getBackend().getTotalFilesize(oids) , 2 ) + " "
									
									: playlist.getTotalClips() + " " + GuiUtils.getStringLocalized("clips")
										+ " | " + formatTime(playlist.getTotalTime()) 
	...
	{
		Rectangle rect;
		rect = new Rectangle( (int)(mainFrame.getX() + (mainFrame.getWidth()/2) - (dialog.getWidth()/2) ),
													(int)(mainFrame.getY() + (mainFrame.getHeight()/2) - (dialog.getHeight()/2) ),
													(int)dialog.getWidth(),
