...
public class JmDNS
{
        ...
        for (Iterator i = cache.iterator(); i.hasNext();)
        {
            for (DNSCache.CacheNode n = (DNSCache.CacheNode) i.next(); n != null; n = n.next())
            {
                ...
                if (rec.type == DNSConstants.TYPE_SRV)
                {
                	...
                	if (convertToType(rec.name).endsWith(type))
                    {
                		// listener.serviceAdded(new ServiceEvent(this, type, toUnqualifiedName(type, convertToType(rec.name)), null));
                        listener.serviceAdded(new ServiceEvent(this, toFullType(rec.name), toUnqualifiedName(type, convertToType(rec.name)), null));
                    }
                }
            }
        }
    ...
    /**
     * Convert the given string of type "_args._protocol._protocol.<IP>.local."
     * to "_args._protocol._protocol.*.local.".
     * 
     * @param	type
    ...
    public static String convertToType(String type) {
    	...
    	boolean done = false;
    	while(!done) {
    		// replace the IP address in the type by a wildcard *
    		if(ip) {
    		...
    		if(token.equals("_udp") || token.equals("_tcp")) {
    			ip = true;
    		}
    		result += token + ".";  		
    	}
     ...
     */
    void recordUpdated(long now, DNSRecord rec)
    {
        ...
        if (rec.type == DNSConstants.TYPE_PTR || rec.type == DNSConstants.TYPE_SRV)
        {
            ...
            synchronized (this)
            {
                ...
                serviceListenerList = (List) serviceListeners.get(convertToType(rec.name.toLowerCase()));
                // Iterate on a copy in case listeners will modify it
                        ...
                        for (Iterator iterator = in.questions.iterator(); iterator.hasNext();)
                        {
                            ...
                            if (entry instanceof DNSQuestion)
                            {
                                ...
                                { // I am not sure of why there is a special case here [PJYF Oct 15 2004]
                                    ...
                                    {
                                    	// assuming queries of type "_protocol._protocol.*.local." (as only service
                                    	// servers should query this JmDNS instance)
                                    	// support for more advanced querying (given subdomain) should start here with
                                    	// convertToType(q.getName().toLowerCase());
                                    ...
                                    case DNSConstants.TYPE_PTR:
                                        {
                                            ...
                                            for (Iterator serviceIterator = services.values().iterator(); serviceIterator.hasNext();)
                                            {
                                                ...
                                                if (info.getState() == DNSState.ANNOUNCED)
                                                {
                                                	// support for more advanced querying should be further
                                                	// implemented here
                                                	// if (q.name.equalsIgnoreCase(info.type))
                                                    if (q.name.equalsIgnoreCase(convertToType(info.type)))
                                                    {
                                                        DNSRecord answer = localHost.getDNS4AddressRecord();
                                                        if (answer != null)
                                                        {
